//
//  HCFacebookManager+UserData.swift
//  iOSTemplate
//
...
// MARK: - Users data extension for HCFacebookManager
extension HCFacebookManager
{
    ...
    open func getFBUserData(requestFields: [FBUserDataField] = [.UserID, .Name, .FirstName, .LastName, .Email, .Gender, .Hometown, .ProfilePicture], success:@escaping (HCFacebookUser?) -> Void, failed:@escaping (Error) -> Void)
    {
        ...
        graphRequest.start {
            ...
            case .success(let graphResponse):
                if let userDict = graphResponse.dictionaryValue
                {
                    ...
                    success(HCFacebookUser(id: userId, name: userName, firstName: "", lastName: "", hasPicture: false, picture: userPicture, homeTown: userHomeTown, email: userEmail))
                }
                success(nil)
            }
        }
