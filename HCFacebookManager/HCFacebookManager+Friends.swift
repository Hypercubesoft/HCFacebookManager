//
//  HCFacebookManager+Friends.swift
//  iOSTemplate
//
...
// MARK: - Friends extension for HCFacebookManager
extension HCFacebookManager
{
    ...
    open func getAllFriends(success:@escaping ([HCFacebookUser]) -> Void, failed:@escaping (Error) -> Void)
    {
    ...
    open func getFriends(success:@escaping ([HCFacebookUser], _ nextCursor:String?) -> Void, failed:@escaping (Error) -> Void, afterCursor:String? = nil)
    {
        let url = "/me/taggable_friends"
        var params = ["fields": "id, first_name, last_name, name, email, picture.width(200).height(200)"]
    ...
    ///   - success: Success completion block if fetching friend list was successful
    open func handleGraphResponseForFriendsList(graphResponse:GraphResponse, success:@escaping ([HCFacebookUser], _ nextCursor:String?) -> Void)
    {
        var friendsList = [HCFacebookUser]()
        if let friendsDict = graphResponse.dictionaryValue as [String : Any]?
        {
            if let friendsListArray = friendsDict["data"] as? [[String:Any]]
            {
                for singleFacebookUserDict in friendsListArray
                {
                    ...
                    friendsList.append(HCFacebookUser(id: userId, name: userName, firstName: userFirstName, lastName: userLastName, hasPicture: hasPicture, picture: userPicture, homeTown: "", email: ""))
                }
            }
