//
//  HCFacebookManager+Images.swift
//  iOSTemplate
//
...
// MARK: - Albums and images extension for HCFacebookManager
extension HCFacebookManager
{
    ...
    open func getAllAlbums(success:@escaping (_ albums:[HCFacebookAlbum]) -> Void, failed:@escaping (Error) -> Void)
    {
        let url = "/me/albums"
        ...
        if let albumsDict = graphResponse.dictionaryValue
        {
            if let albumsArray = albumsDict["data"] as? [[String:Any]]
            {
                for singleAlbumDict in albumsArray
                {
                    ...
                    albums.append(HCFacebookAlbum(albumId: albumId, albumName: albumName, albumCount:albumCount, albumCoverPhotoId: albumCoverPhotoId))
                }
            }
    ...
    open func getAllAlbumImages(albumId:String, success:@escaping (_ pictures:[HCFacebookImage]) -> Void, failed:@escaping (Error) -> Void)
    {
    ...
    ///   - success: Success completion block if fetching album pictures was successful
    ///   - failed: Error completion block if fetching album pictures was unsuccessful
    open func getAlbumImages(albumId:String, success:@escaping (_ pictures:[HCFacebookImage], _ nextCursor:String?) -> Void, failed:@escaping (Error) -> Void)
    {
        let url = "/\(albumId)/photos"
    ...
    open func handleGraphResponseForPicturesList(graphResponse:GraphResponse, success:@escaping ([HCFacebookImage], _ nextCursor:String?) -> Void)
    {
        var pictures = [HCFacebookImage]()
        if let picturesDict = graphResponse.dictionaryValue
        {
            if let picturesArray = picturesDict["data"] as? [[String:Any]]
            {
                for singlePictureDict in picturesArray
                {
                    ...
                    pictures.append(HCFacebookImage(imageId: pictureId, imageUrl: pictureUrl))
                }
            }
