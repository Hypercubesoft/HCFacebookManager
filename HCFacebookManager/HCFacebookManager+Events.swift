//
//  HCFacebookManager+Events.swift
//  iOSTemplate
//
...
extension HCFacebookManager
{
    ...
    open func getAllEvents(accessToken: String = "", facebookPageID: String, success:@escaping ([HCFacebookEvent]) -> Void, failed:@escaping (Error) -> Void)
    {
    ...
    ///   - success: Success function.
    ///   - failed: Failed function.
    open func getEvents(accessToken: String = "", pageLimit: Int = 5, afterCursor: String = "", facebookPageID: String, success:@escaping ([HCFacebookEvent], _ nextCursor:String?) -> Void, failed:@escaping (Error) -> Void)
    {
        var accToken = accessToken
        if accessToken == ""
        {
    ...
    open func handleGraphResponseForEventsList(graphResponse:GraphResponse, success:@escaping ([HCFacebookEvent], _ nextCursor:String?) -> Void)
    {
        var events = [HCFacebookEvent]()
            ...
            if let eventsDicts = eventsDict["data"] as? [[String: Any]]
            {
                for eventDict in eventsDicts
                {
                    ...
                    let startTime:Date = {
                        ...
                        {
                            if let startTime = Date.hcDateFromString(startTimeString, stringFormat: "yyyy'-'MM'-'dd'T'HH':'mm':'ss+SSSS")
                            {
                                return startTime
                            }
                        }
                    ...
                    let endTime:Date = {
                        ...
                        {
                            if let endTime = Date.hcDateFromString(endTimeString, stringFormat: "yyyy'-'MM'-'dd'T'HH':'mm':'ss+SSSS")
                            {
                                return endTime
                            }
                        }
                    ...
                    events.append(HCFacebookEvent(eventId: eventId, eventName: eventName, startTime: startTime, endTime: endTime, isCanceled: isCanceled, eventDescription: eventDescription, placeName: placeName, eventLocation: eventLocation, street: street, country: country, city: city, eventPicture: eventPicture, eventPictureId: eventPictureId))
                }
            }
    ...
    ///   - success: Success function.
    ///   - failed: Failed function.
    open func getNextEventsPage(accessToken: String = "", pageLimit: Int = 5, facebookPageID: String, nextCursor:String, success:@escaping ([HCFacebookEvent], _ nextCursor:String?) -> Void, failed:@escaping (Error) -> Void)
    {
